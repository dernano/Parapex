...
function warumNicht(lage, platz, id, art) {
  ...
  /*
   * Der Kreis: ein Wappen zuendet nicht in einem Ereignis, das es SELBST
   * ausgeloest hat. Nicht als Nachzuendung, nicht als Kopie, und auch nicht
   * im ganz gewoehnlichen Durchlauf der Reihe.
   *
   * Der letzte Teil ist mit Absicht so hart, und er ist gemessen bezahlt:
   * der Ouroboros laesst die ganze Reihe noch einmal laufen - und lief darin
   * selbst wieder mit. Jede Runde des Rades multiplizierte also die naechste,
   * und mit der Fackel daneben (die je Salve ueber fuenf Prozente draufgibt)
   * wuchs das exponentiell in die Tiefensperre hinein. Eine gierige Suche
   * fand in drei Wappen eine Salve von 10^15 gegen einen Heerfuehrer mit
   * viertausend.
   *
   * Die Sperren hatten das im Griff - es war endlich, es brauchte zwei
   * Millisekunden. Das SPIEL hatte es nicht im Griff. Jetzt heisst
   * "die Reihe laeuft noch einmal" genau das: noch EIN Mal.
   */
  if (lage.herkunft.includes(id)) return 'kreis';
  return null;
}
